...
"▐                   ▐           ▐                               ▐    ▐     ▐      "  
    ...
    for idx, option in enumerate(options):
        if idx == selected_option:
            stdscr.addstr(curses.LINES // 2 + 3 + idx, (max_width - len(option)) // 2, option, curses.A_REVERSE)  
        else:
    ...
    while True:
        ...
        if key == curses.KEY_UP and selected_option > 0:
            ...
            if selected_option == 0:
                ...
                ball_x = random.randint(1, curses.COLS - 2)  
                ball_y = curses.LINES - 2  
                ball_dx = 1  
                ball_dy = -1  

                paddle_x = curses.COLS // 2 - 3  
